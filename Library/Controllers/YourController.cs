...
namespace Library.Controllers
{
    public class YourController : Controller
    {
        ...
        public YourController(LibraryContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Index(int? page, string searchbook, string? currentFilter, List<Genre> Genre, int[] idG, List<Author> Author, int[] idA, List<Year> Year, int[] idY, List<Publisher> Publisher, int[] idP,string selectSort)
        {
            ...
            if (user != null)
            {
                books = books.Where(book => book.UserId == user.UserId);
            }
            else books = books.Where(book => book.UserId == 0);
            ...
            IQueryable<Genre> genres = from genre in _context.Genres select genre;
            IQueryable<Author> authors = from author in _context.Authors select author;
            IQueryable<Year> years = from year in _context.Years select year;
            IQueryable<Publisher> publishers = from publisher in _context.Publishers select publisher;
        ...
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> Index(int? page, string searchbook, string? currentFilter, List<Genre> Genre, int[] idG, List<Author> Author, int[] idA, List<Year> Year, int[] idY, List<Publisher> Publisher, int[] idP, string selectSort, string selectOption)
        {
            if (selectSort == null) selectSort = ViewBag.Sort;
